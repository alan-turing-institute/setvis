...
class PlotMissingData(MissingData):
    ...
    def dict_to_matrix(self):
        ...
        the matrix/column data source format required for the plots.
        """
        fields = list(self.missing_fields['Field'][self.missing_fields['Field'] !=0 ])
        # fields = list()
        combinations_matrix = pd.DataFrame(0, index=np.arange(len(self.missing_combinations)), columns=fields)

        index = 0
        for combination, rows in self.missing_combinations.items():
            count = len(rows)
            for field_name in combination:
                combinations_matrix.loc[index, field_name] = count # TODO: at instead count?
            index += 1
    ...
    def combination_heatmap(self, selected_indices=[]):
        ...
        p = figure(title="Combination heatmap", width=960, height=960, tools=TOOLS, # 
                y_range=[str(x) for x in np.arange(self.long_combinations_matrix['index'].max() +1 ) ] )
